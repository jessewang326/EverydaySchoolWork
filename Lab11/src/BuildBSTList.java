...
public class BuildBSTList {
	...
	{		
		// your code goes here	
		System.out.println("start=" + tempArr[start] + ", end=" + tempArr[end] + ", middle=" + (tempArr[start] + (tempArr[end]-tempArr[start])/2));
		if(tempArr[start] ==  tempArr[end])
			bstList.add(null);

		else if ( tempArr[start] ==  tempArr[end -1])
		{
			if (!bstList.contains(tempArr[start]))
				bstList.add(tempArr[start]);
			if (!bstList.contains(tempArr[end]))
				bstList.add(tempArr[end]);
		}

		else{
			Integer middle = tempArr[start] + (tempArr[end]-tempArr[start])/2;
